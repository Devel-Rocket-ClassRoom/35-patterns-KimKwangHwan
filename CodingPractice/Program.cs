...
{
    Console.WriteLine($"Janet: {IsJanetOfJohn("Janet")}");
    Console.WriteLine($"John: {IsJanetOfJohn("John")}");
    Console.WriteLine($"Mike: {IsJanetOfJohn("Mike")}");
}
...

{
    Person[] people = new Person[]
    {
        ...
        new Employee { Name = "철수", Age = 17, Company = "서울고" },
        new Person { Name = "철수", Age = 17 },
    };

    foreach (var p in people)
    {
        Console.WriteLine($"{p.Name}: {DescribDetailed(p)}");
    }
}
...
bool IsJanetOfJohn(string name) => name.ToUpper() is var upper && (upper == "JANET" || upper == "JOHN");
...
string DescribDetailed(Person p) => p switch
{
